...
const BackgroundStyler: React.FC<BackgroundStylerProps> = ({ children, weather }) => {
    ...
    useEffect(() => {
        if (weather) {
            if (weather.condition === "cloudy") {
                ...
                setAdditionalEffect("radial-gradient(circle at left top, rgb(200, 200, 200) 25%, transparent 75%)");
                ...
                setAdditionalEffect("radial-gradient(circle at left top, rgb(200, 200, 70) 5%, transparent 50%)");
            } else if (weather.condition === "windy") {
                setBackgroundGradient('from-[#bbbbbb] to-[#cccccc]');
                setAdditionalEffect("radial-gradient(circle at left top, rgb(250, 250, 250) 1%, transparent 30%)");
            }
        } else {
            // default
    ...
    return (
        <div className={`relative rounded-lg overflow-hidden bg-gradient-to-br ${backgroundGradient}`}>
            <div className="absolute top-0 bottom-0 left-0 right-0" style={{ background: additionalEffect }}></div>
            {children}
        </div>
    );
}
